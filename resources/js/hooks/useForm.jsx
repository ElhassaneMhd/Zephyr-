...
export function useForm({ fields: defFields, defaultValues: def, gridLayout, onSubmit, submitOnEnter }) {
  ...
  const formInputs = useMemo(() => {
    ...
    fields
      ...
      .forEach((field) => {
        const { name, type, placeholder, label, rules, customComponent, format, calculate } = field;
        let value = calculate ? calculate(values) : values?.[name];
        ...
        inputs[name] = customComponent ? (
          <Custom
            ...
            {...{
              field,
              onChange: (value) => {
                validate(name, value, getRules(name, label, type, rules));
                setValue(name, value);
              },
              setValue,
              getValue,
  ...
  function setValue(name, value) {
    return setValues((prev) => {
      ...
      // Calculate the automatic values
      const calculated = fields
        .filter((f) => f.calculate)
        .map((f) => {
          const v = f.calculate(values);
          return { [f.name]: f.format?.(v) || v };
        })
        .reduce((acc, curr) => {
          acc = { ...acc, ...curr };
          return acc;
  ...
  return {
    Form: (
      <form
        className={`grid w-full gap-x-5 gap-y-3 ${gridLayout ? 'md:grid-cols-2' : ''} `}
        onSubmit={(e) => {
          e.preventDefault();
          handleSubmit();
        }}
...
const Input = ({ type, name, ...props }) => {
  const filteredProps = filterObject(props, ['format', 'rules', 'calculate', 'visible'], 'exclude');
