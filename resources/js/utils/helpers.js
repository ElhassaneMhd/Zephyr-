...
export const formatDate = (date, includeTime) => {
    ...
    return getIsoDate(date)
        .setLocale("fr")
        .toLocaleString(
            includeTime ? DateTime.DATETIME_SHORT : DateTime.DATE_FULL,
        );
};
...
export const filterObject = (obj, keys, keysType) => {
    ...
    for (const key in obj) {
        if (keysType === "include" && keys.includes(key))
            filtered[key] = obj[key];
        if (keysType === "exclude" && !keys.includes(key))
            filtered[key] = obj[key];
    }
    return filtered;
};
...
export const getProgress = (ratio) =>
    +(ratio ? (ratio % 1 === 0 ? Math.floor(ratio) : ratio.toFixed(1)) : 0);
...
export const getIntervals = (
    ...
    return intervals
        ...
        .map((interval) => ({
            ...
            value: {
                value: interval,
                condition: (el) => checkDateInIntervals(el[key], interval),
            },
            checked: false,
        }));
};
...
export const getFilter = (name, data, field) => {
    ...
    return {
        [name]: data
            ? data.map((c) => ({
                  value: field ? c[field] : c,
                  checked: param === (field ? c[field] : c),
                  id: field ? c[field] : c,
              }))
            : [],
    };
